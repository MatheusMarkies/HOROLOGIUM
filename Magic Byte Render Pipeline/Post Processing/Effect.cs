...
namespace MagicByte
{
    ...
    class Tonemapping : Effect
    {
        ...
        public override void renderPasses(PostProcessingStack postProcessingStack, CommandBuffer buffer, int fromID,int toID,Camera camera)
        {
            ...
            buffer.GetTemporaryRT(Shader.PropertyToID("_PostFXSource"), camera.pixelWidth, camera.pixelHeight, 0, FilterMode.Bilinear, RenderTextureFormat.DefaultHDR);

            //this.toID = Shader.PropertyToID("_Tonemapping");
            if (this.effectMaterial)
            {
                postProcessingStack.drawingEffect(fromID, toID, this.effectMaterial, 0);
